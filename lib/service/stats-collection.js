const { Connection, ApiClient, constants } = require('chaingreen-api');
...
const chaingreenDashboardUpdater = require('./chia-dashboard-updater');
const chaingreenConfig = require('../chia-config');
const Capacity = require('../capacity');
const ChainGreenAmount = require('../chia-amount');
...
class StatsCollection {
  constructor() {
    ...
    this.updateStatsThrottled = throttle(async partialStats => {
      this.partialStats = {};
      await chaingreenDashboardUpdater.updateStats(partialStats);
  ...
  async init() {
    ...
    const chaingreenConfig = new chaingreenConfig(config.chaingreenConfigDirectory);
    await chaingreenConfig.load();
    this.origin = 'chaingreen-dashboard-satellite';
    const daemonAddress = config.chiaDaemonAddress || chaingreenConfig.daemonAddress;
    const daemonSslCertFile = await chaingreenConfig.getDaemonSslCertFile();
    const daemonSslKeyFile = await chaingreenConfig.getDaemonSslKeyFile();
  ...
  async updateWalletStats() {
    ...
    walletStats.wallets = await Promise.all(wallets.map(async wallet => {
      ...
      return {
        ...
        type: wallet.type,
        balance: {
          confirmed: ChainGreenAmount.fromRaw(balance.confirmed_wallet_balance).toString(),
          spendable: ChainGreenAmount.fromRaw(balance.spendable_balance).toString(),
          unconfirmed: ChainGreenAmount.fromRaw(balance.unconfirmed_wallet_balance).toString(),
        },
      };
    }));
    ...
    walletStats.farmedAmount = {
      farmedAmount: ChainGreenAmount.fromRaw(farmedAmount.farmed_amount).toString(),
      poolRewardAmount: ChainGreenAmount.fromRaw(farmedAmount.pool_reward_amount).toString(),
      farmerRewardAmount: ChainGreenAmount.fromRaw(farmedAmount.farmer_reward_amount).toString(),
      feeAmount: ChainGreenAmount.fromRaw(farmedAmount.fee_amount).toString(),
